...
void syscall_exit(int status)
{
  ...
  if (is_thread_alive(cur->parent) && cur->child_process)
  {
    if (status < 0)
    {
      status = -1;
...
// exucutes command and returns pid of thread executing the command
pid_t syscall_exec(const char *cmdline)
{
  ...
  // check if proccess failed to load
...
bool syscall_create(const char *file_name, unsigned starting_size)
{
  ...
  bool tmp = filesys_create(file_name, starting_size); // from filesys.h
...
{
  lock_acquire(&file_system_lock);
  bool tmp = filesys_remove(file_name); // from filesys.h
  lock_release(&file_system_lock);
  return tmp;
}
...
// helper to check if pointer is valid
void validate_ptr(const void *vaddr)
{
...
void remove_child_process(struct child_process *child_process)
{
...
// add file to list and return file descriptor
int add_file(struct file *file_name)
{
...
// get file that matcches file descriptor
struct file *
