...
// A finction to find thread based on tid
static void find_tid(struct thread *t, void *aux);

// thread which are looking for
static struct thread *matching_thread;

// tid of the thread we are looking for
...
tid_t process_execute(const char *file_name)
{
  ...
  // copy the entire command, including args
  ...
  // new thread to execute program
  tid = thread_create(name, PRI_DEFAULT, start_process, fn_copy);

  // unable to create thread, so free its pages and exit
  if (tid == TID_ERROR)
  {
...
start_process(void *file_name_)
{
  ...
  sema_up(&thread_current()->child_process->load_sema);
  /* If load failed, quit. */
  if (!success)
  {
    thread_exit();
...
void process_exit(void)
{
  ...
  // close the current file, if it is executable
  if (cur->executable)
  {
...
bool load(const char *file_name, void (**eip)(void), void **esp, char **saveptr)
{
  ...
  *eip = (void (*)(void))ehdr.e_entry;
  ...
  /* if the load was successful, then we must ensure that it is not writen to.
...
setup_stack(void **esp, char **saveptr, const char *filename)
{
  ...
  char **command = malloc(DEFAULT_ARGV * sizeof(char *));

  int i, argc = 0;
  int byte_size = 0;
  int arg_size = DEFAULT_ARGV;
  // copy command line into command and resize to necessary size
  for (token = (char *)filename; token != NULL; token = strtok_r(NULL, " ", saveptr))
  {
    ...
    {
      arg_size *= 2;
      command = realloc(command, arg_size * sizeof(char *));
      argv = realloc(argv, arg_size * sizeof(char *));
    }
  }
  // copy content of command over to argv
  for (i = argc - 1; i >= 0; i--)
  {
    *esp -= strlen(command[i]) + 1;
    byte_size += strlen(command[i]) + 1;
    argv[i] = *esp;
    memcpy(*esp, command[i], strlen(command[i]) + 1);
  }
  // add null
  argv[argc] = 0;
